...
- (void)drawRect:(CGRect)rect {
    ...
    if ((_CIImage != nil || _sampleBufferHolder.sampleBuffer != nil) && [self loadContextIfNeeded]) {
        if (self.context.type == SCContextTypeCoreGraphics) {
            ...
            if (image != nil) {
                float rate = MIN(rect.size.width / image.extent.size.width, rect.size.height / image.extent.size.height);
                CGFloat height = image.extent.size.height * rate;
                CGFloat width = image.extent.size.width * rate;
                CGRect frame = CGRectMake((rect.size.width - width) / 2, (rect.size.height - height) / 2, width, height);
                [_context.CIContext drawImage:image inRect:frame fromRect:image.extent];
            }
        }
    }
}
