...
- (void)mixMusicAudio:(AVMutableCompositionTrack *)compositionTrack
{
    ...
    if ([audioAsset tracksWithMediaType:AVMediaTypeAudio].count != 0) {
        track = [audioAsset tracksWithMediaType:AVMediaTypeAudio][0];
    }
    else {
        return;
    }
    
    CMTime videoDuration = compositionTrack.timeRange.duration;
    CMTime startTime = CMTimeMakeWithSeconds(0, videoDuration.timescale);
    CMTime trackDurationTime = CMTimeMakeWithSeconds(FLT_MAX, videoDuration.timescale);
    ...
    CMTime availableTrackDuration = CMTimeSubtract(videoDuration,
                                                   CMTimeMakeWithSeconds(0, videoDuration.timescale));
